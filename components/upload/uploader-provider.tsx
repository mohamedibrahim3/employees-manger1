...
export function UploaderProvider<TOptions = unknown>({
      ...
      await Promise.all(
        filesToUpload.map(async (fileState) => {
          try {
            ...
            // محاكاة التقدم للحصول على تجربة أفضل
            let currentProgress = 0;
            const progressInterval = setInterval(() => {
              if (currentProgress < 90) {
                currentProgress += Math.random() * 10;
                updateFileState(fileState.key, { progress: Math.min(currentProgress, 90) });
              }
            }, 200);
            ...
            // تحديد النهاية بـ 100%
            updateFileState(fileState.key, { progress: 100 });
  ...
  const cancelUpload = React.useCallback(
    (key: string) => {
      ...
      if (fileState?.abortController && fileState.status === 'UPLOADING') {
        fileState.abortController.abort();
        if (fileState?.autoUpload) {
          removeFile(key);
        } else {
          updateFileState(key, { status: 'PENDING', progress: 0, error: undefined });
        }
      }
    },
